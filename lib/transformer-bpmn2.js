...
Transformer.prototype.transform = function (sourceDOM, executable, callback) {
    var innerCallback = function (err, bpmnDefinitions) {
        if (typeof callback == 'function') {
            setImmediate(callback, err, bpmnDefinitions);
        }
    };
    ...
    if (typeof sourceDOM != 'object') {
        return innerCallback(error('Source DOM is missing'));
    }

    var doc = sourceDOM;
    var definitions;
    try {
        definitions = doc.getElementsByTagNameNS(NS_BPMN_SEMANTIC, "definitions");
    } catch (e) {
        return innerCallback(e);
    }

    if (definitions.length === 0) {
        return innerCallback(error("A BPMN 2.0 XML file must contain at least one definitions element"));
    }
    ...
    function transformProcess(processElement, bpmnDiElementIndex, cb) {
        ...
        try {
            ...
            return setImmediate(cb, null, bpmnObject);
        } catch (err) {
    ...
    function transformDefinitions(definitionsElement, cb) {
        ...
        async.each(processes, function (process, eachCb) {
            var name = processNames[process.getAttribute("id")];
            if (name) {
                process.setAttributeNS(NS_BPMN_SEMANTIC, "name", processNames[process.getAttribute("id")]);
            }
            transformProcess(process, bpmnDiElementIndex, eachCb);
            ...
            var bpmnReturn = generatedElements.concat(messageFlows, dataAssociations, messages);

            return cb(null, bpmnReturn);
        });
    }

    // return transformDefinitions(definitions[0], callback);
    transformDefinitions(definitions[0], innerCallback);
};
